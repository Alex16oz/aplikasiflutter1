...
class _SchedulePageState extends State<SchedulePage> {
  ...
  Future<void> _showScheduleDialog(
    ...
    try {
      ...
      if (isEditing) {
        final assignedOperatorsData = await _supabase
            .from('schedule_operators')
            .select('operator_id')
            .eq('schedule_id', schedule['schedule_id']);
        selectedOperatorIds = (assignedOperatorsData as List)
            .map((row) => row['operator_id'] as String)
            .toSet();

        final machineName = schedule['machine_name'];

        final machineMap = (machinesData).firstWhere(
              (m) => m['machine_name'] == machineName,
          orElse: () => <String, dynamic>{},
        );
        machineIdForEditing = machineMap['id'];
      }

      if (mounted) Navigator.of(context).pop();

      final formKey = GlobalKey<FormState>();
      final int? initialSelectedMachineId = isEditing
          ? machineIdForEditing
          : (damageReport?['machine_id'] as int?);
        ...
        builder: (BuildContext dialogContext) {
          ...
          return StatefulBuilder(
            builder: (context, setDialogState) {
              return AlertDialog(
                title:
                Text(isEditing ? 'Edit Jadwal' : 'Buat Jadwal Baru'),
                content: Form(
                  key: formKey,
                      ...
                      children: [
                        DropdownButtonFormField<int>(
                          ...
                          items: (machinesData)
                              ...
                              value: m['id'],
                              child: Text(m['machine_name'] ?? 'Unknown')))
                              .toList(),
                          onChanged: (value) =>
                              setDialogState(() => currentMachineId = value),
                          validator: (v) =>
                          v == null ? 'Mesin harus dipilih' : null,
                        ),
                        ...
                        const Text('Alokasikan Operator',
                          ...
                          child: ListView(
                            children: (operatorsData).map((op) {
                              ...
                              return CheckboxListTile(
                                title:
                                Text(op['username'] ?? 'Unknown Operator'),
                        ...
                        if (selectedOperatorIds.isEmpty)
                          Padding(
                            ...
                            child: Text(
                                'Pilih setidaknya satu operator',
                                style: TextStyle(
                  ...
                  ElevatedButton(
                    onPressed: () async {
                      if (formKey.currentState!.validate() &&
                          selectedOperatorIds.isNotEmpty) {
                        try {
                          final scheduleData = {
                          ...
                          if (isEditing) {
                            ...
                            await _supabase
                                ...
                                .delete()
                                .eq('schedule_id', schedule['schedule_id']);
                            final operatorRecords = selectedOperatorIds
                                .map((opId) => {
                            ...
                            final newSchedule = await _supabase
                                .from('schedules')
                                .insert({
                              ...scheduleData,
                          ...
                          if (mounted) {
                            ScaffoldMessenger.of(context).showSnackBar(
                                SnackBar(
                                    content: Text(
                                        'Jadwal berhasil ${isEditing ? 'diperbarui' : 'dibuat'}!'),
                                    backgroundColor: Colors.green));
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content: Text("Error preparing form: $e"),
            backgroundColor: Colors.red));
      }
    }
  }
              ...
              child: SingleChildScrollView(
                ...
                child: DataTable(
                  ...
                  rows: schedules.map((schedule) {
                    final operatorNames =
                    List<String>.from(schedule['operator_names'] ?? []);
                    final scheduleStatus = schedule['status'] ?? 'Dijadwalkan';

                    return DataRow(
                      cells: <DataCell>[
                        DataCell(Text((schedule['schedule_date'] != null)
                            ? DateFormat('dd MMM yy')
                            .format(DateTime.parse(schedule['schedule_date']))
                            : 'N/A')),
                        DataCell(Text(schedule['machine_name'] ?? 'N/A')),
                        DataCell(Text(operatorNames.isNotEmpty
                            ? operatorNames.join(', ')
                            : 'Belum dialokasikan')),
                        DataCell(
                          Chip(
                        ...
                        DataCell(Row(
                          children: [
                            if (scheduleStatus == 'Selesai' ||
                                scheduleStatus == 'Terverifikasi')
                              IconButton(
                            ...
                            // --- PERBAIKAN KONDISI TOMBOL HAPUS ---
                            if (isAdmin && (scheduleStatus == 'Dijadwalkan' || scheduleStatus == 'Scheduled'))
                              IconButton(
                                ...
                                onPressed: () =>
                                    _deleteSchedule(schedule['schedule_id']),
                              ),
                          ],
                        )),
                      ],
  ...
  Color _getStatusColor(String status) {
    switch (status) {
      ...
      case 'Scheduled': // Menambahkan case untuk status dari screenshot
